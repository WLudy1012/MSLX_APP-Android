...
class LocalServerManager(
    ...
    suspend fun start(dirName: String): Result<Unit> {
        ...
        val serverDir = LocalInstanceStore.dir(app, dirName)
        ...
        if (!LocalJreManager.isInstalled(app, runtime)) {
            return Result.failure(IllegalStateException("请先在「设置 → 本机运行时」安装 ${runtime.label}"))
        }
        ...
        if (fit == LocalJreManager.RuntimeFit.TOO_OLD) {
            ...
            val guidance = if (needed == null || !needed.supported) {
                ...
                "请到「设置 → 本机运行时」安装 ${needed.label}，再把本实例运行时切过去"
            }
        ...
        val extraArgs = meta.jvmArgs.trim().split(Regex("\\s+")).filter { it.isNotBlank() }
        // 启动前补全实例文件（幂等、不覆盖用户改动），拿最新的展示名/内存等
        val finalMeta = ServerFiles.complete(serverDir, meta).getOrElse { meta }
        ...
        return LocalServerRuntime.start(
            ...
            serverJar = serverJar,
            workDir = serverDir,
            serverName = finalMeta.name.ifBlank { meta.name.ifBlank { dirName } },
            minMemM = finalMeta.minMemMb,
            maxMemM = finalMeta.maxMemMb,
            extraArgs = extraArgs,
            useSerialGc = finalMeta.useSerialGc,
            keepAlive = finalMeta.keepAlive,
            runtime = runtime,
            useExec = useExec,
        )
    }
